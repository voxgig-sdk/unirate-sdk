...
boru-driven command-line client **and** interactive REPL for the Unirate
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unirate-cli)
make build
...
./unirate-cli load 1 commodity            # {id:1} shorthand
./unirate-cli load '{id:1}' commodity       # explicit match map

# 5. Override the API base URL for a single call
UNIRATE_BASE=https://api.example.com ./unirate-cli load 1 commodity

# 6. No arguments -> interactive REPL
./unirate-cli
unirate> load 1 commodity
unirate> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unirate-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/unirate-cli load 1 commodity
   ```
...
```sh
./unirate-cli load 1 commodity          # scalar shorthand for {id:1}
./unirate-cli load '{id:1}' commodity     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `unirate>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `commodity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/unirate-cli`. |
...
(`./unirate-cli <expr>`) and interactively (the REPL), and expressions compose the
