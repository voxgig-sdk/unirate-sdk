// Command Unirate-cli is a CLI + REPL for the Unirate SDK,
...
const prompt = "unirate"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "commodity currency historical_currency vat_rate"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UNIRATE_APIKEY (api key), UNIRATE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UNIRATE_APIKEY carries the API key and
	// UNIRATE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UNIRATE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UNIRATE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUnirateSDK(opts)
