...
class FtCtrl extends UnirateControl
{
...
class FtClient
{
    ...
    private UnirateUtility $utility;

    public function __construct(UnirateUtility $utility, array $options)
    {
    ...
    public function get_utility(): UnirateUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UnirateContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new UnirateUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (UnirateContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UnirateConfig::make_feature($name);
    ...
    private static function buildUrl(UnirateSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UnirateError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UnirateResponse($fetched);
        }
    ...
    private function fail(UnirateContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UnirateContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
