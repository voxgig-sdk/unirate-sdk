...
The Python SDK for the Unirate API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Commodity(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Unirate(None).load(
    {"id": "test01"}, None
)
