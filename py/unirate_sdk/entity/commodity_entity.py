...
class CommodityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CommodityEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Commodity:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Commodity().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
