...
class TestVatRateDirect:

    def test_should_direct_load_vat_rate(self):
        setup = _vat_rate_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-vat_rate", "live" if setup["live"] else "unit")
        if _skip:
...
def _vat_rate_direct_setup(mockres):
