...
The Lua SDK for the Unirate API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Commodity(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Unirate(nil):load(
  { id = "test01" }, nil
)
