...
local VatRateEntity = {}
VatRateEntity.__index = VatRateEntity


function VatRateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VatRateEntity)
  self._name = "vat_rate"
  self._client = client
...
function VatRateEntity:get_name()
  return self._name
end


function VatRateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VatRateEntity.new(self._client, opts)
end
...
function VatRateEntity:mark_deleted()
  self._deleted = true
end


function VatRateEntity:deleted()
  return true == self._deleted
end


function VatRateEntity:data_set(args)
  if args ~= nil then
...
function VatRateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VatRateEntity:match_set(args)
  if args ~= nil then
...
function VatRateEntity:match_get()
...
function VatRateEntity:stream(action, args, callopts)
...
function VatRateEntity:_run_op(ctx, post_done)
  local utility = self._utility
