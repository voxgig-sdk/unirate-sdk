# Unirate Lua SDK Reference

Complete API reference for the Unirate Lua SDK.
...
```lua
local result, err = client:Commodity():load({ api_key = "api_key" })
```
...
```lua
local result, err = client:Currency():load({ api_key = "api_key" })
```
...
```lua
local result, err = client:HistoricalCurrency():load({ api_key = "api_key" })
```
...
```lua
local result, err = client:VatRate():load({ api_key = "api_key" })
```
