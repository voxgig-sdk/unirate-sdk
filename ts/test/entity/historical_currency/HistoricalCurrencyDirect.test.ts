...
describe('HistoricalCurrencyDirect', async () => {
  ...
  test('direct-load-historical_currency', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-historical_currency', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['UNIRATE_TEST_HISTORICAL_CURRENCY_ENTID']
