...
//     (\`new UnirateSDK(...)\` / \`UnirateSDK.test(...)\` become
//     \`UnirateSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"commodity":{"test01":{"id":"test01"}},"currency":{"test01":{"id":"test01"}},"historical_currency":{"test01":{"id":"test01"}},"vat_rate":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UnirateSDK(...)\` and any
// \`UnirateSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UnirateSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UnirateSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
