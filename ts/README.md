# Unirate TypeScript SDK



The TypeScript SDK for the Unirate API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const commodity = await client.Commodity().load({ id: 'example_id' })
...
// commodity is a bare entity populated with mock response data
console.log(commodity)
```
...
| `tester(testopts?, sdkopts?)` | `UnirateSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `UnirateSDK.test(testopts?, sdkopts?)` | `UnirateSDK` | Create a test-mode client. |
...
| `client` | `client(): UnirateSDK` | Return the parent SDK client. |
