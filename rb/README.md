...
The Ruby SDK for the Unirate API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Commodity(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Unirate(nil).load(
  { "id" => "test01" }, nil
)
