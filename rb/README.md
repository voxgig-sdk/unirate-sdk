...
The Ruby SDK for the Unirate API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Commodity` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = UnirateSDK.test(testopts, sdkopts)
```
...
raise a `UnirateError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Unirate_sdk.rb       -- Main SDK module
...
The main module (`Unirate_sdk`) exports the SDK class
