...
class HistoricalCurrencyEntity
  ...
  def make
    opts = @_entopts.dup
    HistoricalCurrencyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UnirateHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [HistoricalCurrency, Hash] the current HistoricalCurrency data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UnirateHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of HistoricalCurrency fields)
  def match_get
  ...
  #
  # @param reqmatch [HistoricalCurrencyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.HistoricalCurrency.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [HistoricalCurrency, Hash] the loaded HistoricalCurrency; raises UnirateError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UnirateHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
