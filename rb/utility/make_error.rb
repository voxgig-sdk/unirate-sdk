...
module UnirateUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = UnirateContext.new({}, nil)
    end
    op = ctx.op || UnirateOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || UnirateResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(UnirateError) ? err.msg : err.to_s
    ...
    sdk_err = UnirateError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(UnirateError)
