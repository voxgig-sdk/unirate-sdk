...
type Args struct {
	Entity string         `json:"entity" jsonschema:"commodity | currency | historical_currency | vat_rate"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.UnirateSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "unirate_list",
		Description: "List records from Unirate. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "unirate_load",
		Description: "Load a single record from Unirate. " +
...
func runOp(client *sdk.UnirateSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.UnirateSDK, name string) (sdk.UnirateEntity, error) {
	switch strings.ToLower(name) {
	case "commodity":
		return client.Commodity(nil), nil
	case "currency":
		return client.Currency(nil), nil
	case "historical_currency":
		return client.HistoricalCurrency(nil), nil
	case "vat_rate":
		return client.VatRate(nil), nil
