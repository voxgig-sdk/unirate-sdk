...
[MCP](https://modelcontextprotocol.io) server exposing the Unirate SDK as
two agent tools — `unirate_list` and `unirate_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unirate-mcp)
make build

# 2. Provide credentials via the environment
export UNIRATE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user unirate \
  -- /absolute/path/to/unirate-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./unirate-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// unirate_list: first page of records
{ "entity": "commodity" }
{ "entity": "commodity", "query": { } }

// unirate_load: one record by id
{ "entity": "commodity", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unirate-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user unirate \
     -- "$PWD"/dist/*/unirate-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `unirate_list` and `unirate_load` tools now appear
   in new sessions. Ask the agent to *"list commodity using unirate"*
   and it calls `unirate_list` with `{"entity":"commodity"}`.
...
```sh
export UNIRATE_APIKEY=sk_live_xxx            # API key
export UNIRATE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `unirate_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `unirate_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UNIRATE_APIKEY` | API key sent with every request. |
| `UNIRATE_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

commodity | currency | historical_currency | vat_rate

### Smoke test via HTTP (raw JSON-RPC)

```sh
./unirate-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"unirate_load","arguments":{"entity":"commodity","query":{"id":1}}}}'
```
