...
import (
	...
	omni "github.com/voxgig-sdk/unirate-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UnirateError); is {
				out := map[string]any{
					"name":    "UnirateError",
...
func omniCtx(arg any, client *sdk.UnirateSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UnirateSDK); is {
			client = live
