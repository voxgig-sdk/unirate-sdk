...
Learn more about UniRate API at [unirateapi.com](https://unirateapi.com).
...
This SDK exposes the API as a small set of **semantic entities** — Commodity, Currency, HistoricalCurrency and VatRate — that you
...
// commodity is the Commodity entity, populated with mock data
...
// Load a specific commodity (returns the ENTITY; call data_get() for the record; throws on error)
$commodity = $client->Commodity()->load(["api_key" => "example_api_key"]);
print_r($commodity);
```
...
# Load a specific commodity (returns the ENTITY; call data_get for the record)
commodity = client.Commodity.load({ "api_key" => "example_api_key" })
puts commodity
```
