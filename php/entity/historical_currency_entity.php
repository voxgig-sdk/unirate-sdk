...
// Unirate SDK HistoricalCurrency entity
...
class HistoricalCurrencyEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "historical_currency";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new HistoricalCurrencyEntity($this->_client, $opts);
    }

    /**
     * @param HistoricalCurrency|array $args HistoricalCurrency data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UnirateHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return HistoricalCurrency|array The current HistoricalCurrency data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of HistoricalCurrency fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UnirateHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of HistoricalCurrency fields).
     */
    ...
    /**
     * Load a single HistoricalCurrency.
     *
     * @param HistoricalCurrencyLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed HistoricalCurrencyLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return HistoricalCurrency|array The loaded HistoricalCurrency as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = UnirateHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
