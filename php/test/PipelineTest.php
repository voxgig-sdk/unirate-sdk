...
{
    private static function utility(): UnirateUtility
    {
        return new UnirateUtility();
    }
    ...
    private static function ctx(array $over = []): UnirateContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UnirateContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UnirateOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UnirateError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UnirateOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UnirateMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UnirateOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UnirateMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UnirateOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UnirateMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnirateSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UnirateMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UnirateResponse([]),
            'result' => new UnirateResult([]),
        ]);
        [, $err] = UnirateMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new UnirateSpec([]),
            'response' => null,
            'result' => new UnirateResult([]),
        ]);
        [, $err] = UnirateMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UnirateSpec(['step' => 's']),
            'response' => new UnirateResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UnirateResult([]),
        ]);
        [, $err] = UnirateMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UnirateSpec(['step' => 's']),
            'response' => new UnirateResponse(self::resp(200, ['v' => 1])),
            'result' => new UnirateResult([]),
        ]);
        [, $err] = UnirateMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UnirateSpec(['step' => 's']),
            'response' => new UnirateResponse(self::resp(200, ['v' => 2])),
            'result' => new UnirateResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UnirateMakeResponse::call($ctx);
    ...
    {
        $preset = new UnirateResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UnirateSpec([]),
            'response' => new UnirateResponse([]),
            'result' => new UnirateResult([]),
        ...
        [$got, $err] = UnirateMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UnirateResult([])]);
        [, $err] = UnirateMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UnirateSpec([]), 'result' => null]);
        [, $err] = UnirateMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnirateSpec(['step' => 's']),
            'result' => new UnirateResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UnirateOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnirateMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnirateSpec(['step' => 's']),
            'result' => new UnirateResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UnirateOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnirateMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnirateResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UnirateSpec([]),
            'result' => new UnirateResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UnirateMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UnirateMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UnirateError('boom', 'boom');
        $utility->fetcher = function (UnirateContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnirateSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UnirateMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UnirateContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UnirateMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UnirateResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UnirateContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnirateSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UnirateMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UnirateResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UnirateSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UnirateMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UnirateMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UnirateMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UnirateResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UnirateDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UnirateResult(['ok' => false])]);
        $this->expectException(UnirateError::class);
        UnirateDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UnirateResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UnirateMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        UnirateMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UnirateBaseFeature();
        $b = new UnirateBaseFeature();
        UnirateFeatureAdd::call($ctx, $a);
        UnirateFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UnirateBaseFeature
    {
        $f = new UnirateBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UnirateFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UnirateContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UnirateSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UniratePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UnirateResponse(['headers' => null]),
            'result' => new UnirateResult([]),
        ]);
        UnirateResultHeaders::call($ctx);
