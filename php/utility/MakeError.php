...
class UnirateMakeError
{
    public static function call(?UnirateContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UnirateContext([], null);
        }
        $op = $ctx->op ?? new UnirateOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UnirateResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UnirateError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UnirateError('', $msg, $ctx);
