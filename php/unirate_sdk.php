...
class UnirateSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = UnirateHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = UnirateHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!UnirateFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, UnirateFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return UnirateUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = UnirateHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = UnirateHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = UnirateHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new UnirateError($op . "_allow",
                "UnirateSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = UnirateHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new UnirateError("graphql_error",
                "UnirateSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
